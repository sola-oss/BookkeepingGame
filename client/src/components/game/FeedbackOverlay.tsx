...
import type { Account, EntrySide } from "@shared/schema";
import { entrySideLabels, categoryLabels } from "@shared/schema";
...
export function FeedbackOverlay({ feedback, onComplete }: FeedbackOverlayProps) {
  const { account, selectedSide, correctSide, isCorrect, scoreChange } = feedback;
        ...
        <div className="flex flex-col items-center gap-4 text-center">
          ...
          {!isCorrect && (
            <div className="text-sm text-muted-foreground">
              <p>
                <span className="line-through">{entrySideLabels[selectedSide]}</span>
                {" → "}
                <span className="font-bold text-foreground">{entrySideLabels[correctSide]}</span>
              </p>
            </div>
          )}
