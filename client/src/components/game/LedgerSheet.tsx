  ...
  return (
    <Card className="bg-white dark:bg-zinc-950 p-2 sm:p-4 rounded-xl border shadow-sm font-sans overflow-hidden">
      ...
      <div className="overflow-x-auto -mx-2 sm:mx-0 border-t border-b border-zinc-800 dark:border-zinc-200">
        <Table className="min-w-[500px] border-collapse text-[10px] sm:text-[11px]">
          <TableHeader className="bg-zinc-50 dark:bg-zinc-900/50">
            <TableRow className="hover:bg-transparent border-b border-zinc-800 dark:border-zinc-200 h-8">
              <TableHead className="w-12 border-r text-center font-bold px-0.5">
              ...
              <TableHead className="border-r text-center font-bold px-1">摘 要</TableHead>
              <TableHead className="w-10 border-r text-center font-bold text-[9px] px-0.5">仕丁</TableHead>
              <TableHead className="w-20 border-r text-center font-bold px-1">借 方</TableHead>
              <TableHead className="w-20 border-r text-center font-bold px-1">貸 方</TableHead>
              <TableHead className="w-8 border-r text-center font-bold text-[9px] px-0.5">借/貸</TableHead>
              <TableHead className="w-24 text-center font-bold px-1">残 高</TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            {rows.map((row, idx) => (
              <TableRow key={idx} className="hover:bg-zinc-100/50 dark:hover:bg-zinc-800/30 h-7 border-b border-zinc-200 dark:border-zinc-800 border-dotted last:border-solid last:border-zinc-800">
                ...
                <TableCell className="border-r px-1.5 truncate max-w-[120px]">{row.memo}</TableCell>
                <TableCell className="border-r text-center text-[9px] text-muted-foreground p-0">{row.sourceEntryId}</TableCell>
                <TableCell className="border-r text-right px-1.5 font-mono">
                  {row.debit > 0 ? row.debit.toLocaleString() : ""}
                </TableCell>
                <TableCell className="border-r text-right px-1.5 font-mono">
                  {row.credit > 0 ? row.credit.toLocaleString() : ""}
                </TableCell>
                <TableCell className="border-r text-center font-bold text-[9px] p-0">
                  {row.balanceSide}
                </TableCell>
                <TableCell className="text-right px-1.5 font-mono font-bold">
